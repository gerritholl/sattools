...
def test_get_fs_and_files(sS, tmp_path, monkeypatch, sector):
    ...
    tf = (tmp_path / "noaa-goes16" / f"ABI-L1b-Rad{sector:s}" / "1900" / "001" / "00" /
     f"OR_ABI-L1b-Rad{sector:s}-M6C1_G16_s19000010005000_e19000010010000_"
     "c20303212359590.nc")
    ...
    assert fns == [
            FileInfo(
                path = str(tmp_path / "noaa-goes16" / f"ABI-L1b-Rad{sector:s}" / "1900" /
